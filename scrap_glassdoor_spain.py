...
def clean_salary(list_of_salaries):
    """
    Clean the string format of the web salary
    :param list_of_salaries
    :return: Salary list (numbers)
    """
    for index in range(len(list_of_salaries)):
        if "mil" and "-" in list_of_salaries[index]:
            bad_format_salary = []
...
def set_browser():
    """
    Set up a Chrome browser pre-logged in gmail
...
def check_pages_main_page(pages):
    """
    Check if there are more pages to continue in the main page, where there are all the companies
    :param pages: String format of the web pages
...
def check_company_pages(pages):
    """
    Check if there are more pages to continue in the company page, where there are all the jobs and salaries
    :param pages: String format of the company pages
    :return: True if there are more pages
    """
    pages_list = pages.text.split(" ")
...
def get_company_info(driver):
    """
    Get the rating, the name, and the sector of each company
    :param driver: Chrome driver
    :return: Three list, with the three mention parameters cleaned
    """
    ...
    company_rating_list = [e for e in company_rating if e.text[0].isdigit()]
...
def get_company_df(driver, company_name, rating, sector):
    """
    Creates dataframe with the company info and gets each job and salary of the company to create the dataframe
    :param driver: Chrome driver
    :param company_name: Name of the company
    :param rating: Rating of company
    ...
    while door:
        try:
            ...
            for salary_job in job_info:
                match = re.search('€/año$', salary_job.text)
...
def set_company_salary_page(driver, company_name):
    """
    Sets the company page where there are all the jobs and salaries
    :param driver: Chrome driver
    :param company_name: Name of the company
    :return: int
    """
...
def set_page_to_get_companies_info(driver):
    """
    Sets the type of the companies we want to scrap
    :param driver: Chrome driver
    """
    time.sleep(5)
...
if __name__ == '__main__':
    ...
    page_url = "https://www.glassdoor.es/Opiniones/index.htm"
    ...
    for i in range(len(final_companies_list)):
        browser_driver.get(page_url)
        counter = set_company_salary_page(browser_driver, final_companies_list[i])
        time.sleep(2)
        if counter == 3:
            continue
        else:
    ...
    final_df.to_csv('glassdoor_datasets(csv)/spain_jobs_salaries_by_company.csv', index=False)
